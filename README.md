...
Backend (Express server) — Handles API requests, caching, rate-limiting, and fallback data logic.

Frontend (React app) — Displays live index cards and charts using Chart.js.
...
The server checks if a cached entry exists and whether it’s still valid.

If valid → returns data directly from memory.

If expired or missing → makes a new API request, stores the response in cache, and returns it.

If the vendor API fails (403/429/500), the server gracefully serves the last cached snapshot or fallback mock data.
...
Anurag Jindal
Seattle University | M.S. Computer Science
LinkedIn
 • GitHub
